...
const (
	...
	// the API's own default upload limit so anything attach_document managed to
	// upload can be read back; it is a guard against an absurd object, not a
	// policy about document size. The API decides that, and says so by name
	// when a file is too large.
	maxDocumentBytes = 12 << 20

	// maxSourceBytes bounds the Markdown itself, on both ends of the pair.
	//
	...
	// reason — a model handed a silently shortened source would write the next
	// version from it and delete the rest of the document for good.
	//
...
func (t *server) registerDocuments(s *mcp.Server) {
	mcp.AddTool(s, &mcp.Tool{
		...
		Description: "Attach a Markdown document to a card as one self-contained HTML file, " +
			"replacing the previous one of the same name that this token uploaded. " +
			"Name it with filename — that is what tells two documents on the same card " +
			"apart, and what read_document asks for; it defaults to design.html. " +
			"The Markdown is typeset for a person to read and kept verbatim inside the " +
			"file, so read_document returns exactly what you wrote. The card's title " +
			"already heads the document, so do not repeat it. For a design document, " +
			"write the reasoning behind the work — what was chosen, what was rejected " +
			"and why, what was assumed, what is still open — rather than a description " +
			"of what the code does, which is read from the code.",
		Annotations: replaces(),
	}, t.attachDocument)

	mcp.AddTool(s, &mcp.Tool{
		Name: "read_document",
		Description: "Read back the Markdown source of a document attached to a card by " +
			"attach_document. Give the filename it was written under; it defaults to " +
			"design.html. Call it before writing over one, so a second pass builds on " +
			"the first instead of starting over. Answers with the source, never with a link.",
...
type attachDocumentIn struct {
	...
	Markdown string `json:"markdown" jsonschema:"the document source; Markdown including tables and fenced code"`
	Filename string `json:"filename,omitempty" jsonschema:"what to call it on the card; defaults to design.html, and must end in .html"`
}
...
func (t *server) attachDocument(ctx context.Context, _ *mcp.CallToolRequest, in attachDocumentIn) (*mcp.CallToolResult, *documentView, error) {
	...
	// Read the card before uploading anything. It names the document for
	// whoever opens it out of their Downloads folder, and it fails early and
	// cheaply on a card id that does not exist or belongs to another team.
	card, err := t.api.GetCard(ctx, in.Card)
	if err != nil {
		return nil, nil, err
	}
	...
	// anything else that lands on the card meanwhile.
...
//
// Two conditions before anything is deleted, because this is the only delete in
// the whole server and it runs on a model's say-so:
//
//   - the same account uploaded it. The token acts as a real member, so a file
//     of this name from somebody else is their work.
//   - the file carries a Markdown source block, which is to say attach_document
//     wrote it. Uploader alone is not enough: the same person could have put a
//     file of this name on the card by hand through the web UI, and that is a
//     document this tool has no business deleting either.
//
...
func (t *server) removeSuperseded(ctx context.Context, previous []noeto.Attachment, filename, us string) (int, string) {
	var replaced, foreign, unknownOwner, notOurs, unreadable int
	var failures []string
	...
	for _, old := range previous {
		...
		ours, err := t.supersedes(ctx, old)
		switch {
		case err != nil:
			// Could not fetch it, so there is nothing to base a delete on.
			// Kept separate from "not ours" because the note has to be true:
			// this file may well be one of ours.
			unreadable++
			continue
		case !ours:
			notOurs++
	...
	if notOurs > 0 {
		notes = append(notes, copies(notOurs, filename)+" carrying no Markdown source block, so attach_document did not write them; left alone")
	}
	if unreadable > 0 {
		notes = append(notes, copies(unreadable, filename)+" that could not be fetched to check, left alone")
	}
	if len(failures) > 0 {
...
// supersedes reports whether the new document replaces this attachment, by
// fetching it and looking for the sealed source block.
//
// One extra request before a delete, which is the right trade: the normal case
// is a single previous copy of our own document, and the alternative is
// deleting a file on the strength of its name.
//
// A missing object counts as replaceable. The API answers a presign failure
// with an empty download link and a 404 means the object is simply not there —
// two very different things that both stop the check. The second one is a row
// pointing at nothing, so removing it destroys nothing; treating it as unknown
// instead would mean a card that accumulates a fresh copy on every call and
// never sheds the dead one.
func (t *server) supersedes(ctx context.Context, a noeto.Attachment) (bool, error) {
	raw, err := t.api.DownloadAttachment(ctx, a, maxDocumentBytes)
	if noeto.NotFound(err) {
		return true, nil
	}
	if err != nil {
	...
	if _, err := extractSource(string(raw)); err != nil {
		return false, nil
	}
	return true, nil
}
...
type readDocumentIn struct {
	Card     string `json:"card" jsonschema:"the card id, from get_board or find_cards"`
	Filename string `json:"filename,omitempty" jsonschema:"the name it was attached under; defaults to design.html"`
}
...
func (t *server) readDocument(ctx context.Context, _ *mcp.CallToolRequest, in readDocumentIn) (*mcp.CallToolResult, *documentSourceView, error) {
	...
	raw, err := t.api.DownloadAttachment(ctx, *found, maxDocumentBytes)
	if err != nil {
		return nil, nil, err
	}

	source, err := extractSource(string(raw))
	if err != nil {
		return nil, nil, err
	}
	if len(source) > maxSourceBytes {
		// Deliberately not truncated: a shortened source looks complete, and
		// the next attach_document would write it back and lose the remainder.
		return nil, nil, eris.Errorf(
			"%s holds %d bytes of Markdown, past the %d-byte limit — open the attachment in a browser instead",
			found.Filename, len(source), maxSourceBytes)
	}
	...
	if candidates > 1 {
		// Anybody on the team can put a file on a card, so the newest copy of
		// this name is not necessarily the one the last attach_document wrote.
		// Only one can be returned; saying that a choice was made is what keeps
		// the substitution from being invisible.
...
// it. Hence: .html, because that is what makes a double click open it typeset;
// and no colon, because macOS shows a colon in a filename as a slash in the
// Finder and the file becomes hard to talk about.
func documentFilename(raw string) (string, error) {
	...
	if !strings.EqualFold(filepath.Ext(name), ".html") {
		return "", &badInput{field: "filename", want: "a name ending in .html", got: raw}
	}
	return name, nil
}

// sameName compares filenames the way the people looking at them do. The API
// stores whatever case it was given, so "Design.html" and "design.html" are two
...
// called "deſign.html" would answer to "design.html" while looking like
// something else in the listing. Nothing legitimate needs that.
